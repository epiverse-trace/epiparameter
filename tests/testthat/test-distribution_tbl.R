...
test_that("distribution_tbl works as expected supplied with db", {
  dist_tbl <- distribution_tbl(multi_epidist = edist)
...
test_that("distribution_tbl works for incubation period with db", {
  incub_tbl <- distribution_tbl(
    multi_epidist = edist,
...
test_that("distribution_tbl works for different distribution with db", {
  serial_tbl <- distribution_tbl(
    multi_epidist = edist,
...
test_that("distribution_tbl works for COVID-19 db", {
  incub_tbl <- distribution_tbl(
    multi_epidist = edist,
    disease = "COVID-19"
  )
...
test_that("distribution_tbl works for SARS-CoV-2 db", {
  incub_tbl <- distribution_tbl(
    multi_epidist = edist,
    pathogen = "SARS-CoV-2"
  )
...
test_that("distribution_tbl works for disease & epi_dist subset with db", {
  incub_tbl <- distribution_tbl(
    multi_epidist = edist,
    disease = "COVID-19",
    epi_dist = "incubation period"
...
test_that("distribution_tbl works when supplied a subset db", {
  edist <- suppressMessages(epidist_db(disease = "COVID-19"))
  covid_tbl <- distribution_tbl(multi_epidist = edist, disease = "COVID-19")
...
test_that("distribution_tbl works as expected supplied with <epidist>", {
...
test_that("distribution_tbl works as expected with discretised <epidist>", {
...
test_that("distribution_tbl fails correctly when subsetting a subset db", {
  edist <- suppressMessages(epidist_db(disease = "Ebola"))
  expect_error(
    distribution_tbl(multi_epidist = edist, disease = "COVID-19"),
    regexp = "(distribution not available for COVID-19)"
  )
})

test_that("distribution_tbl fails correctly", {
  ...
  expect_error(
    distribution_tbl(multi_epidist = edist, epi_dist = "random"),
    regexp = "random distribution not available"
  )

  # check for multiple match input
  expect_error(
    distribution_tbl(multi_epidist = edist, epi_dist = "onset"),
