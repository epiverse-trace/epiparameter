test_that("convert_params works as expected", {
  expect_equal(
    convert_params(distribution = "lnorm", meanlog = 1, sdlog = 0.5),
    list(
  ...
  expect_equal(
    convert_params(distribution = "nbinom", prob = 0.5, dispersion = 1),
    list(
  ...
  expect_equal(
    convert_params(distribution = "weibull", shape = 1, scale = 1),
    list(
      mean = 1, median = 0.6931472, mode = 0, var = 1, sd = 1, cv = 1,
  ...
  expect_equal(
    convert_params(distribution = "geom", prob = 0.5),
    list(
      mean = 1, median = 0, mode = 0, var = 2, sd = 1.414214, cv = 1.414214,
      skewness = 2.12132, ex_kurtosis = 6.5
    ),
...
test_that("convert_summary_stats works as expected", {
  expect_equal(
    convert_summary_stats(distribution = "gamma", mean = 1, sd = 0.5),
  ...
  expect_equal(
    suppressMessages(
      convert_summary_stats(distribution = "weibull", mean = 0.5, cv = 1)
    ),
  ...
  expect_equal(
    convert_summary_stats(distribution = "lnorm", median = 1, sd = 1),
    list(meanlog = -0.3465736, sdlog = 0.8325546),
    tolerance = 1e-4
  )

  expect_equal(
    convert_summary_stats(distribution = "nbinom", mean = 1, dispersion = 1),
    list(prob = 0.5, dispersion = 1),
    tolerance = 1e-4
  )

  expect_equal(
    convert_summary_stats(distribution = "nbinom", mean = 1, sd = 1.5),
    list(prob = 0.4444444, dispersion = 0.8),
    tolerance = 1e-4
  )

  expect_equal(
    convert_summary_stats(distribution = "geom", mean = 0.5),
    list(prob = 0.6666667),
    tolerance = 1e-4
  )
})

test_that("convert_params fails as expected", {
  expect_error(
    convert_params(distribution = "random", shape = 1, scale = 1),
    regexp = "(arg)*(should be one of)*(lnorm)*(gamma)"
  )

  expect_error(
    convert_params(distribution = "lnorm"),
    regexp = "(Parameter(s) need to be named and supplied)*(to the function)"
  )

  expect_error(
    convert_params(distribution = "lnorm", meanlog = 1, meanlog = 1),
    regexp = "(Parameter(s) need to be named and supplied)*(to the function)"
  )
  ...
  expect_error(
    convert_params(distribution = "lnorm", shape = 1, scale = 1),
    regexp = "(lnorm parameters must be named)*(meanlog)*(sdlog)"
  )

  expect_error(
    convert_params(distribution = "gamma", meanlog = 1, scale = 1),
    regexp = "(gamma parameters must be named)*(shape)*(scale)"
  )

  expect_error(
    convert_params(distribution = "weibull", meanlog = 1, scale = 1),
    regexp = "(weibull parameters must be named)*(shape)*(scale)"
  )

  expect_error(
    convert_params(distribution = "nbinom", probability = 0.5, dispersion = 1),
    regexp = "(nbinom parameters must be named)*(prob)*(dispersion)"
  )

  expect_error(
    convert_params(distribution = "geom", probability = 0.5),
    regexp = "(geom parameter must be named)*(prob)"
  )
 })

test_that("convert_summary_stats fails as expected", {
  expect_error(
    convert_summary_stats(distribution = "random", mean = 1, sd = 1),
    regexp = "(arg)*(should be one of)*(lnorm)*(gamma)"
  )

  expect_error(
    convert_summary_stats(distribution = "lnorm"),
    regexp = "Summary statistics need to be named and supplied to the function"
  )

  expect_error(
    convert_summary_stats(distribution = "lnorm", mean = 1, mean = 1),
    regexp = "Summary statistics need to be named and supplied to the function"
  )

  expect_error(
    convert_summary_stats(distribution = "gamma", mean = -1, sd = 1),
    regexp = "(Assertion on)*(x$mean)*(failed: Element 1 is not >= 0.)"
  )

  expect_error(
    convert_summary_stats(distribution = "lnorm", mode = 1, var = 1),
    regexp = "Cannot calculate lognormal parameters from given input"
  )

  expect_error(
    convert_summary_stats(distribution = "gamma", mode = 1, var = 1),
    regexp = "Cannot calculate gamma parameters from given input"
  )

  expect_error(
    convert_summary_stats(distribution = "weibull", mode = 1, var = 1),
    regexp = "Cannot calculate Weibull parameters from given input"
  )

  expect_error(
    convert_summary_stats(distribution = "nbinom", mode = 1, var = 1),
    regexp = "(Cannot calculate negative binomial)*(parameters from)*(input)"
  )

  expect_error(
    convert_summary_stats(distribution = "nbinom", mean = 1, var = 0.5),
    regexp = "(Negative binomial)*(variance-to-mean ratio)*(greater than one)"
  )

  expect_error(
    convert_summary_stats(distribution = "geom", mode = 1, var = 1),
