...
test_that("convert_params_to_summary_stats works as expected with dyn-dots", {
  ...
  expect_identical(
    ...
    convert_params_to_summary_stats(
      "nbinom", !!!list(prob = 0.5, dispersion = 1)
    )
  )

  expect_identical(
    convert_params_to_summary_stats("gamma", shape = 1, scale = 1),
    convert_params_to_summary_stats("gamma", !!!list(shape = 1, scale = 1))
  )

  expect_equal(
    convert_params_to_summary_stats("weibull", shape = 1, scale = 1),
    convert_params_to_summary_stats("weibull", !!!list(shape = 1, scale = 1))
  )

  expect_equal(
    convert_params_to_summary_stats("geom", prob = 0.5),
    convert_params_to_summary_stats("geom", !!!list(prob = 0.5))
  )
})
...
test_that("convert_summary_stats_to_params works as expected with dyn-dots", {
  ...
  expect_identical(
    ...
    suppressMessages(
      convert_summary_stats_to_params("weibull", !!!list(mean = 0.5, cv = 1))
    )
  )

  expect_identical(
    convert_summary_stats_to_params("lnorm", median = 1, sd = 1),
    convert_summary_stats_to_params("lnorm", !!!list(median = 1, sd = 1))
  )

  expect_identical(
    convert_summary_stats_to_params("nbinom", mean = 1, dispersion = 1),
    convert_summary_stats_to_params("nbinom", !!!list(mean = 1, dispersion = 1)
    )
  )

  expect_identical(
    convert_summary_stats_to_params("nbinom", mean = 1, sd = 1.5),
    convert_summary_stats_to_params("nbinom", !!!list(mean = 1, sd = 1.5))
  )

  expect_identical(
    convert_summary_stats_to_params("geom", mean = 0.5),
    convert_summary_stats_to_params("geom", !!!list(mean = 0.5))
  )
})
