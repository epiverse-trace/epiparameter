...
The `{epiparameter}` package will act as a 'living systematic review' (*sensu* @elliott_living_2014) which will be actively updated and maintained to provide the most comprehensive and reliable source of data on epidemiological distributions.
...
However, these simple search phrases can return a large number of irrelevant papers. We recommend using more specific search schema depending on the search engine used. For example, if using Google Scholar a schema like:
...
- ("COVID-19" OR "SARS-CoV-2") AND "onset to death" AND estimat*
