...
- `<epiparam>`: library of epidemiological parameters
- `<epidist>`: singular set of epidemiological parameters 
- `<vb_epidist>`: a singular set of epidemiological parameters for a vector-borne disease containing both an extrinsic and intrinsic distribution. This object contains two sets of parameters, one for the human (intrinsic) and one for the vector (extrinsic).
...
For binding columns to `<epiparam>` use either `tibble::add_column()` or `dplyr::bind_cols()`. Using `cbind()` will unclass the `<epiparam>` object (i.e. convert an `<epiparam>` to a `<data.frame>`). Note that `dplyr::bind_cols()` will print a message that a `<data.frame>` is being returned, but this is not the case.
