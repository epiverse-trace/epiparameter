...
```{r, parameter-tbl}
parameter_tbl(multi_epiparameter = epi_dist_db)
```

`parameter_tbl()` can also subset the database supplied to the function.

```{r, parameter-tbl-subset}
parameter_tbl(multi_epiparameter = epi_dist_db, disease = "Ebola")
```
...
covid_incubation <- epiparameter(
  ...
  summary_stats = create_epiparameter_summary_stats(mean = 2),
