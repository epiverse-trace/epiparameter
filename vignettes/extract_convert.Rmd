...
Use conversions when possible over extraction to avoid possible limitations associated with
...
two arguments as input and convert to two variables as output (e.g. `gamma_shapescale2meansd()`).
...
All currently supported summary statistic conversion to parameters for the gamma distribution are given below. We aim to expand these to include conversion from other combinations of summary statistics.

```{r}
convert_gamma_summary_stats(mean = 2, sd = 2)
convert_gamma_summary_stats(mean = 2, var = 2)
convert_gamma_summary_stats(mean = 2, cv = 2)
convert_gamma_summary_stats(mode = 2, sd = 2)
convert_gamma_summary_stats(mode = 2, var = 2)
```
...
convert_lnorm_summary_stats(mean = 2, median = 2)
```
...
A message is shown when running `extract_param()` to make the user aware that the estimates are not completely reliable due to the use of numerical optimisation and that the user should re-run the function to make sure that
the convergence on the parameters returned from the function can be repeatedly found. This issue is mostly overcome by the internal setup of the `extract_param()` function which searches for convergence to consistent parameter estimates before returning these to the user.
...
The reasoning for the maximum iterations is the amount of time the function could take cycling through optimisation routines without finding a consistent answer. If the runtime is not important and parameter accuracy is paramount then the maximum number of iterations can be increased and tolerance decreased.

This `control` argument is the same whether extracting from percentiles or range.

### Use cases

Here we present a few example of published epidemiological parameters and distribution where the functions outlined above can be applied to get the parameters of the distribution.
...
# Mpox lnorm from 75% percentiles in WHO data
extract_param(
  type = "percentiles",
  values = c(6, 13),
  distribution = "lnorm",
