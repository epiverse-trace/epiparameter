...
marburg_params <- epireview::get_parameter(marburg_params, parameter_name = "Human delay - incubation period")
marburg_params
```

We will select the first entry, which is an incubation period, to use as the first example:

```{r, select-marburg-entry}
marburg_incub <- marburg_params[1, ]
marburg_incub
```
...
In general, all required values for a parameter are represented as a _single_ entry in epireview. In a small number of cases, particularly for Marburg Virus Disease (as the first pathogen we extracted), values captured for a parameter are on multiple rows. We are trying to avoid this as _linking_ these entries is very challenging. We provide further information in the limitations section below. 

The way the {epireview} data is stored means that some epidemiological parameter entries may require multiple rows. This can be, for example, because they contain two summary statistics (e.g. mean and standard deviation) that are kept on separate rows. In order to create `<epidist>` objects that contains the full information for each entry multiple rows of the epidemiological parameters table from {epireview} can be given to `as_epidist()` to create a single `<epidist>` object.
...
ebola_params <- epireview::get_parameter(ebola_params, "Human delay - serial interval")
ebola_params
```
